...
class TreeView extends ScrollView {
  ...
  static content() {
    return this.div({
      ...
      'data-use-dock-integration': atom.config.get('Remote-FTP-Plus.tree.useDockIntegration'),
    }, () => {
      this.div({
        class: 'scroller',
        outlet: 'scroller',
  ...
  initialize(...args) {
    ...
    atom.config.onDidChange('Remote-FTP-Plus.tree.hideLocalWhenDisplayed', (values) => {
      if (values.newValue) {
        if (this.isVisible()) {
          hideLocalTree();
        }
    ...
    atom.config.onDidChange('Remote-FTP-Plus.tree.useDockIntegration', () => {
      if (typeof atom.workspace.getRightDock === 'undefined') {
        ...
        atom.config.set('Remote-FTP-Plus.tree.useDockIntegration', 'false');
      } else if (this.isVisible()) {
        setTimeout(() => {
          this.detach();
          this.attach();
    ...
    this.getTitle = () => 'Remote-FTP-Plus';
  }

  attach() {
    const enableDock = atom.config.get('Remote-FTP-Plus.tree.useDockIntegration');
    const showOnRightSide = atom.config.get('tree-view.showOnRightSide');
    const hideLocalDisplay = atom.config.get('Remote-FTP-Plus.tree.hideLocalWhenDisplayed');
