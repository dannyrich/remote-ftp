...
import { multipleHostsEnabled, getObject, hasProject, logger, traverseTree, validateConfig, getActiveConfig } from './helpers';
  ...
  class Client extends EventEmitter {
      ...
      self.watch = {
        ...
        addListeners() {
          ...
          .on('change', (path) => {
            ...
            if (atom.config.get('Remote-FTP-Plus.notifications.enableWatchFileChange')) {
    ...
    events() {
      ...
      this.subscriptions.add(
        atom.config.onDidChange('Remote-FTP-Plus.dev.debugResponse', (values) => {
          self.watchDebug(values.newValue);
        }),
        atom.config.onDidChange('Remote-FTP-Plus.tree.showProjectName', () => {
          self.setProjectName();
        }),
      );
    }
    ...
    setProjectName() {
      const self = this;
      const projectRoot = atom.config.get('Remote-FTP-Plus.tree.showProjectName');
    ...
    readConfigAll(callback) {
      if (typeof callback !== 'function') return false;
      const self = this;
      const error = err => callback.apply([], err);

      FS.readFile(self.getConfigPath(), 'utf8', (err, res) => {
    ...
    readConfig(callback) {
      ...
      FS.readFile(self.ftpConfigPath, 'utf8', (err, res) => {
        ...
        if (validateConfig(data)) {
          try {
            json = getActiveConfig(JSON.parse(data));
        ...
        if (json !== null && typeof callback === 'function') {
          const ssconfigPath = atom.config.get('Remote-FTP-Plus.connector.sshConfigPath');
    ...
    onceConnected(onconnect) {
      ...
      console.warn(`Remote-FTP-Plus: Not connected and typeof onconnect is ${typeof onconnect}`);
      return false;
    }
    ...
    doConnect() {
      ...
      switch (self.info.protocol) {
        ...
        default:
          throw new Error('No `protocol` found in connection credential. Please recreate .ftpconfig file from Packages -> Remote-FTP-Plus -> Create (S)FTP config file.');
      }
      ...
      self.watchDebug(atom.config.get('Remote-FTP-Plus.dev.debugResponse'));
    }
