...
const init = () => {
  ...
  const commands = {
    'remote-ftp-plus:create-ftp-config': {
      enabled: true,
      command() {
        createConfig({
          protocol: 'ftp',
    ...
    'remote-ftp-plus:create-sftp-config': {
      enabled: true,
      command() {
        createConfig({
          protocol: 'sftp',
    ...
    'remote-ftp-plus:create-ignore-file': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'remote-ftp-plus:toggle': {
      enabled: true,
      command() {
        remoteftp.treeView.toggle();
      },
    },
    'remote-ftp-plus:connect': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'remote-ftp-plus:refresh-selected': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'remote-ftp-plus:delete-selected': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'remote-ftp-plus:download-selected': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'remote-ftp-plus:download-active': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'remote-ftp-plus:download-selected-local': {
      enabled: true,
      command() {
        if (!hasProject()) return;
        ...
        if (!client.isConnected()) {
          ...
          atom.commands.dispatch(viewWorkspace, 'remote-ftp-plus:connect');

          client.once('connected', () => {
            atom.commands.dispatch(viewWorkspace, 'remote-ftp-plus:download-selected-local');
          });
    ...
    'remote-ftp-plus:upload-selected': {
      enabled: true,
      command() {
        if (!hasProject()) return;
        ...
        if (!client.isConnected()) {
          ...
          atom.project.remoteftp.once('connected', () => {
            atom.commands.dispatch(viewWorkspace, 'remote-ftp-plus:upload-selected');
          });
        ...
        const enableTransfer = atom.config.get('Remote-FTP-Plus.notifications.enableTransfer');
        ...
        locals.forEach((local) => {
          ...
          client.upload(local, (err, list) => {
            ...
            if (err && !/[Ff]ile( already)? exists/.test(err)) {
              console.error(err);
              return;
            }
    ...
    'remote-ftp-plus:upload-active': {
      ...
      command() {
        ...
        client.upload(local, (err, list) => {
          if (err) {
            if (enableTransfer) atom.notifications.addError('Remote FTP: Transfer failed.');
            return;
          }
          ...
          if (enableTransfer) atom.notifications.addSuccess('Remote FTP: Transfers succeeded.');
        });
      },
    },
    // Remote -> Local
    'remote-ftp-plus:sync-with-remote': {
      enabled: true,
      command() {
        const remotes = remoteftp.treeView.getSelected();
        remotes.forEach((view) => {
    ...
    // Local -> Remote
    'remote-ftp-plus:sync-with-local': {
      enabled: true,
      command() {
        if (!hasProject()) return;
        ...
        if (!client.isConnected()) {
          ...
          client.once('connected', () => {
            atom.commands.dispatch(viewWorkspace, 'remote-ftp-plus:sync-with-local');
          });
    ...
    'remote-ftp-plus:update-status-bar': {
      enabled: true,
      command() {
        if (!hasProject()) return;
        const { item } = getStatusBarTile();

        client.readConfig((err, config) => {
          if (err) return;
          let { name } = config;
          const { url, color = 'inherit' } = config;
          ...
          item.querySelector('.remote-ftp-server').textContent = name;
        });
      },
    },
  };
