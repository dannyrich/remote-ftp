...
let modifyStatusBarTile;

export const hasProject = () => atom.project && atom.project.getPaths().length;
export const multipleHostsEnabled = () => atom.config.get('Remote-FTP-Plus.beta.multipleHosts');
...
export const setStatusBarTile = (statusBarTile) => {
  modifyStatusBarTile = statusBarTile
};

export const getStatusBarTile = () => modifyStatusBarTile;
...
export const getActiveConfig = (data) => {
  let dataset;
  let active = atom.config.get('Remote-FTP-Plus.activeConfig');
  ...
  if (dataset instanceof Array) {
    if (isNaN(active) || active > dataset.length) {
      active = 0;
      atom.config.set('Remote-FTP-Plus.activeConfig', 0);
    }
